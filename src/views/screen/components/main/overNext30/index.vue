...
const initDate = (): string[] => {
  ...
  let startDate = dayjs();  // 获取当前日期
  const endDate = startDate.add(30, "day");  // 获取30天后的日期
...
const data = {
  ...
  data: new Array(31).fill("").map(val => {
    val = (Math.random() * 200000).toFixed(0);
    return val;
  })
};
...
.echarts {
  ...
  :deep(.line-chart-bg) {
    ...
    height: 60px;
    padding-left: 20px;
    background: url("@/views/screen/images/line-bg.png") no-repeat;
